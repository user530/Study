...
function t3() {
    ...
    a3_res = [];
    a3.forEach(v=>{if(typeof(v)==`number`)a3_res.push(v)})
...
function t4() {
    a4_res = [];
    let task4 = document.querySelectorAll('span.task-4');
    task4.forEach(v=>a4_res.push(v.getAttribute('data')));
...
function t10() {
    ...
    let URLparams = new URLSearchParams(a10);
    a10_res = `?${URLparams.toString()}`;
