...
//--------------------------------------------------------------------------------------------------------------------
    ...
    for (let i = 0 ; i < canyon.length ; i++)
        {
            draw_canyons(canyon[i]);
            check_canyons(canyon[i]);
            borders_canyons(canyon[i]);
        }
    ...
    for (let i = 0 ; i < collectable.length ; i++)
        {
            draw_collectables(collectable[i]);
            check_collectables(collectable[i]);
        }
...
function draw_canyons(t_canyon)
{
        ...
        strokeWeight(10);
        beginShape();
            vertex(t_canyon.pos_x - 10, t_canyon.pos_y);
            vertex(t_canyon.pos_x + t_canyon.width / 2, 25);
            vertex(t_canyon.pos_x + t_canyon.width + 10, t_canyon.pos_y);
        endShape();

        stroke(0);
        strokeWeight(3);
        triangle(t_canyon.pos_x, 50, 
                    t_canyon.pos_x + t_canyon.width / 2, 25,
                        t_canyon.pos_x + t_canyon.width, 50);

        fill(bg.col_stone);
        rect(t_canyon.pos_x, 50, t_canyon.width, height - floorPos_y);

        //---PIT---
        fill(bg.col_sand_light);
        noStroke();
        rect(t_canyon.pos_x, t_canyon.pos_y - 3 , t_canyon.width, height - floorPos_y + 3);
...
function draw_collectables(t_collectable)
{
        ...
        fill(char.col_accessories);
        beginShape(TRIANGLES);
        vertex(t_collectable.pos_x - t_collectable.size / 10, t_collectable.pos_y);
        vertex(t_collectable.pos_x + t_collectable.size / 10, t_collectable.pos_y);
        vertex(t_collectable.pos_x, t_collectable.pos_y - t_collectable.size / 2);
        vertex(t_collectable.pos_x,
               t_collectable.pos_y - t_collectable.size / 2);
        vertex(t_collectable.pos_x + t_collectable.size / 4,
               t_collectable.pos_y - t_collectable.size / 3);
        vertex(t_collectable.pos_x + t_collectable.size / 4,
               t_collectable.pos_y - t_collectable.size / 2);
        vertex(t_collectable.pos_x,
               t_collectable.pos_y - t_collectable.size / 2);
        ...
        vertex(t_collectable.pos_x,
               t_collectable.pos_y - t_collectable.size / 3);
        vertex(t_collectable.pos_x - t_collectable.size / 4,
               t_collectable.pos_y - t_collectable.size / 1.05);
        vertex(t_collectable.pos_x + t_collectable.size / 4,
               t_collectable.pos_y - t_collectable.size / 1.05);
        endShape(CLOSE);
        ...
        beginShape();
        vertex(t_collectable.pos_x,
               t_collectable.pos_y - t_collectable.size / 1.8);
        vertex(t_collectable.pos_x - t_collectable.size / 9,
               t_collectable.pos_y - t_collectable.size / 1.2);
        vertex(t_collectable.pos_x + t_collectable.size / 9,
               t_collectable.pos_y - t_collectable.size / 1.2);
        endShape(CLOSE);
        }
        ...
        fill(bg.col_stone);
        beginShape();
            vertex(t_collectable.pos_x - 15, floorPos_y);
            vertex(t_collectable.pos_x - 15, floorPos_y - 15);
            vertex(t_collectable.pos_x + 15, floorPos_y - 15);
            vertex(t_collectable.pos_x + 15, floorPos_y);
        endShape(CLOSE);
        ...
        beginShape();
            vertex(t_collectable.pos_x - 15, floorPos_y - 15);
            vertex(t_collectable.pos_x - 25, floorPos_y - 30);
            vertex(t_collectable.pos_x + 25, floorPos_y - 30);
            vertex(t_collectable.pos_x + 15, floorPos_y - 15);
        endShape(CLOSE);
...
function check_canyons(t_canyon)
{
    if ((isPlummeting == false) && 
            (gameCharAbs_x - 7 >= t_canyon.pos_x)&&(gameCharAbs_x + 7 <= t_canyon.pos_x + t_canyon.width))
        {
            isPlummeting = true;
        }
}

function borders_canyons(t_canyon)
{
    if ((gameChar_y > floorPos_y) && 
            (gameCharAbs_x <= t_canyon.pos_x + t_canyon.width) && 
                (gameCharAbs_x >= t_canyon.pos_x))
        {
            if (gameCharAbs_x + 24 > t_canyon.pos_x + t_canyon.width)
            {
                gameCharAbs_x = t_canyon.pos_x + t_canyon.width - 24;
            }
            else if (gameCharAbs_x - 24 < t_canyon.pos_x)
            {
                gameCharAbs_x = t_canyon.pos_x + 24;
            }
        }
}

function check_collectables(t_collectable)
{
    if (dist(gameCharAbs_x, gameChar_y, t_collectable.pos_x, t_collectable.pos_y + 30) <= 25){
