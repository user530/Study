...
function carIsBySide( target_car )
{
    ...
    if ((target_car.coord_x != sleuthPI_car.coord_x) && (abs(target_car.distance_travelled - sleuthPI_car.distance_travelled) < 25))
        {
            return true;
        }
...
.	 - if a positive result is returned then the licence_plate property of the found car is then checked against the criminal description.
	...
	*/
    ...
    for (let i = 0 ; i < vehicles_data.length ; i++)
        {
            carIsBySide(vehicles_data[i]);
            if ((carIsBySide(vehicles_data[i]) == true) && (vehicles_data[i].licence_plate == "DYWQ30"))
                {
                    criminal = carIsBySide(vehicles_data[i]);
                }
        }
}
