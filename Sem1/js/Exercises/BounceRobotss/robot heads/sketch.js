...
function setup()
{
	...
	robot2Speed_x = random(1, 3);
	robot2Speed_y = random(1, 3);
...
function draw()
{
	...
	robot1_x += robot1Speed_x * robot1_vec_X;
	robot1_y += robot1Speed_y * robot1_vec_Y;
    
	robot2_x += robot2Speed_x * robot2_vec_X;
	robot2_y += robot2Speed_y * robot2_vec_Y;

	//place your if statements here
    
    //robot 1 walls reflect
    if ((robot1_x + 107 >= width)||(robot1_x - 17 <= 0)){
        robot1_vec_X = - robot1_vec_X;
        robot1Speed_x = random(1, 3);
        robot1Speed_y = random(1, 3); 
        
    }
    
    if ((robot1_y + 100 >= height)||(robot1_y - 17 <= 0)){
        robot1_vec_Y = - robot1_vec_Y;
        robot1Speed_x = random(1, 3);
        robot1Speed_y = random(1, 3); 
    }
    
    //robot 2 walls reflect
    if ((robot2_x + 107 >= width)||(robot2_x - 17 <= 0)){
        robot2_vec_X = - robot2_vec_X;
        robot2Speed_x = random(1, 3);
        robot2Speed_y = random(1, 3); 
    }
    
    if ((robot2_y + 100 >= height)||(robot2_y - 17 <= 0)){
        robot2_vec_Y = - robot2_vec_Y;
        robot2Speed_x = random(1, 3);
        robot2Speed_y = random(1, 3); 
    }
    
     //robot 1 head reflect
   if (((max(robot1_x, robot2_x) >= min(robot1_x, robot2_x))&&(max(robot1_x, robot2_x) <= min(robot1_x, robot2_x) + 100))&&
       ((max(robot1_y, robot2_y) >= min(robot1_y, robot2_y))&&(max(robot1_y, robot2_y) <= min(robot1_y, robot2_y) + 100))){
        robot1_vec_X = - robot1_vec_X;
        robot1_vec_Y = - robot1_vec_Y;
        robot2_vec_X = - robot2_vec_X;
        robot2_vec_Y = - robot2_vec_Y;
